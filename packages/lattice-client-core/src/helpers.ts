...
export function getCombinedInventoryFromHosts(
  hosts: Record<string, WasmCloudHost> | WasmCloudHost[],
) {
  const inventory: {
  ...
  for (const host of Object.values(hosts)) {
    for (const [key, component] of Object.entries(host.components)) {
      ...
      if (existingComponent === undefined) {
        /* eslint-disable @typescript-eslint/naming-convention -- property is defined externally */
        ...
        /* eslint-enable @typescript-eslint/naming-convention -- property is defined externally */
      } else {
